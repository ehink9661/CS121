 ...
 *
 * List of all the functions used in Assignment #2, using test files to fill a linked list
 *---------------------------------------------------------------------
 */
...
#include <fstream>
#include <string>
#include "animalList.h" // allows for prototypes and struct to be read
...
void print(){
  ...
  p = head;
  while(p != NULL){ // loop to keep printing unitl the head of the list is NULL
    cout << p->animal << ": " << count(p->animal) << endl; // prints how many of an animal are in a list
    p = p->next; // moves to next item in list
  }
  cout << "----------" << endl;
}
...
void remove(string a){
  ...
  if(p == NULL){ // if the list is empty
    ...
    if(curr == NULL){
      cout << "Can not delete " << a << " because it is not contained in the list" << endl;
    }
    else if(curr != NULL){ // runs if a value was found in the list
...
void read(string a){ // reads the text file for commands, the only input being the name of the file
  ...
  while(infile >> word){
    if(word == "add"){ // checks if the input is addd before running the add function
      infile >> word;
      add(word);
    }
    else if(word == "delete" || word == "remove"){ // checks if the input is delete or remove before running the delete function
      infile >> word; // finds the word to be deleted
      remove(word);
    }
    else if(word == "show"){ // checks if the input is show to run the print function
      print();
    }
  }
}

int count(string a){ // takes an input to count how many of an animal are in the list
  int counter = 0; // starts the counter at 0
  NodePtr p = head; // creates pointer p equal to head
  while(p != NULL){ // loop to keep printing unitl the head of the list is NULL
    if(p->animal == a){ // if the animal exists in the list
      counter +=1; // addds one to the counter
    }
    p = p->next; // moves to next item in list
  }
  return counter; // returns the counted number of the string
}

void clear(){ // clears the list for the next list
  while(head != NULL){ // checks if the list has ended
    remove(head->animal); // removes the data in the list
  }
}
